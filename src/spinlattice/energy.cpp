...
namespace sld{
	...
	double compute_potential_energy(const int start_index, 
									const int end_index,
	...
	double compute_kinetic_energy(const int start_index,
								  const int end_index,
								  const std::vector<int>& type_array, 
	...
	double compute_effective_J(const int start_index,
							   const int end_index,
	...
	double compute_effective_C(const int start_index,
							   const int end_index,
	...
	double compute_coupling_energy(const int start_index, 
								   const int end_index)
	{
		double sumC = 0.0;
		for (int at = start_index; at < end_index; at++){
		...
		sumC *= mp::material[0].mu_s_SI / 1.602176634e-19;
		return sumC;
	}

	double compute_exchange_energy(const int start_index,
								   const int end_index)
	{
		double sumJ = 0.0;
		for (int at = start_index; at < end_index; at++){
		...
		sumJ *= mp::material[0].mu_s_SI / 1.602176634e-19;
		return sumJ;
	}
